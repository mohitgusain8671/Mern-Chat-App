  ...
  return (
    <div className='mt-5' >{
        contacts.map((contact)=>(
            <div 
                key={contact._id} 
                className={`pl-10 py-2 transition-all duration-300 cursor-pointer ${selectedChatData && selectedChatData._id === contact._id ? 'bg-[#8417ff] hover:bg-[#8317fff0]' : 'hover:bg-[#f1f1f111]'}`} 
            ...
            >
                <div className="flex gap-5 items-center justify-start text-neutral-300">
                    ...
                    { isChannel ? <span> {contact.name} </span> : <span>{(contact.name ? contact.name : contact.email )}</span>}
                </div>
